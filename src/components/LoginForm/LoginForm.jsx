    ...
    <div onClick={()=>{
      ...
      <div onClick={(e)=>e.stopPropagation()} className="login__form">
        ...
        <div className="login__form-block">
          ...
          {requestError ? <div style={{color:'red'}}>Неверный логин или пароль</div> : null}
          <input
            value={loginValue.username}
            onChange={(e)=>setLoginValue({...loginValue, username:e.target.value})}
        ...
        <div className="login__form-btn-block">
          <button onClick={()=>{
            if(loginValue.password.length == 0 && loginValue.username.length == 0){
              setError(!error)
            }else{

              signInFunction.signIn(loginValue.username, loginValue.password)
